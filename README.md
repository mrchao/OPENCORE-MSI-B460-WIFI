...
|name| default | 说明 | 建议 | 
| -- | ------- | --- | ---- |
|DevirtualiseMmio|false|从某些 MMIO 区域中删除 Runtime 属性| true |
|ResizeAppleGpuBars|-1|减少 GPU PCI BAR 的大小，以便与 MacOS 兼容|0|
|ResizeAppleGpuBars|true|在 SetVirtualAddresses 上设置虚拟内存|false|
|SyncRuntimePermissions|false|更新运行时环境的内存权限|true|

Kernel/Quirks
|name| default | 说明 | 建议 | 
| -- | ------- | --- | ---- |
|CustomSMBIOSGuid|false|自定义模式执行 GUID 修补，通常用于戴尔笔记本电脑|true|
|DisableIoMapper|false|禁用 XNU (VT-d) 中的 IOMapper 支持，这可能与固件的实现相冲突|true|
|DisableIoMapperMapping|false|在 IOMMU(VT-d) 中禁止映射 PCI 桥接设备内存||
|SetApfsTrimTimeout|-1|为 SSD 上的 APFS 文件系统设置微秒级的 trim 超时时间, 设置0为禁用|0|
...
|name| default | 说明 | 建议 | 
| -- | ------- | --- | ---- |
|HideAuxiliary|true|隐藏辅助项|false|
|PickerAttributes|17|开机引导菜单的属性||
|PickerMode|Builtin|引导模式|External|
|PickerVariant|Auto|启动管理器图标路径||

Misc/Security

|name| default | 说明 | 建议 | 
| -- | ------- | --- | ---- |
|DmgLoading|Signed|dmg加载||
|ScanPolicy|17760515|定义操作系统检测策略||
|SecureBootModel|Default|Apple 安全启动的机型|j185|
|Vault|Secure|启用 OpenCore 的 Vault 机制|Optional|
...
- 7C436110-AB2A-4BBB-A880-FE41995C9F82
  ...
  - boot-args
    ...
    > 例：
    > igfxfw=2 igfxonln=1 alcid=1 gfxrst=1 alcdelay=500 shikigva=80 -cdfon -igfxmpc
    ...
    > 例：
    > igfxrpsc=1 igfxonln=1 agdpmod=pikera
    - igfxrpsc=1 启用RPS控制补丁（提高IGPU性能）
    - igfxonln=1 使用核显输出时强制HDMI/DP始终在线，防止无法唤醒出现黑屏
